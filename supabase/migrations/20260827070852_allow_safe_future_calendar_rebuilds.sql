...
begin
  ...
  if exists (
    ...
    where (c.source_semester_id = p_semester_id or c.usable_semester_id = p_semester_id)
      and (
        ...
        or coalesce(
             c.branch_id,
             (select p.branch_id from public.profiles p where p.id = c.student_id)
           ) = p_branch_id
      )
  ) then
    return true;
  ...
  loop
    ...
    if not v_has_materialized then
      continue;
    end if;
    ...
    if not found or v_start <= v_today then
      return true;
    end if;
  end loop;
    ...
    where r.source_semester_id = p_semester_id
      ...
      and (
        ...
        or exists (
          ...
          where ml.manual_makeup_right_id = r.id
        )
        or 1 <> (
          select count(*)
        ...
        or not exists (
          ...
          where l.lesson_right_id = r.id
            ...
            and l.starts_at = l.occurrence_at
    ...
    if exists (
      ...
      where r.student_id = v_plan.student_id
        ...
        and (
          ...
          or exists (
            select 1 from public.lessons ml
            where ml.manual_makeup_right_id = r.id
          )
          or 1 <> (
            select count(*)
      ...
      raise exception using
        errcode='P0001',
        message='FORESTRING_CALENDAR_REBUILD_BLOCKED_BY_USED_LESSON',
...
create or replace function private.reconcile_semester_row_change()
returns trigger
language plpgsql
security definer
set search_path to ''
...
begin
  if tg_op <> 'UPDATE'
     or (old.starts_on is not distinct from new.starts_on and old.ends_on is not distinct from new.ends_on) then
    return new;
  end if;

  if new.code like '__FORESTRING_STAGE__%' or old.code like '__FORESTRING_STAGE__%' then
    return new;
  end if;
  ...
  for v_branch in
    ...
    where not exists (
      ...
      where o.branch_id = b.id
        and o.semester_id = new.id
    )
  loop
    perform private.rebuild_future_regular_semester(new.id, v_branch.id);
  end loop;
...
create or replace function private.reconcile_branch_semester_override_change()
returns trigger
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  if old.starts_on is distinct from new.starts_on
     ...
     or old.semester_id is distinct from new.semester_id then
    perform private.rebuild_future_regular_semester(old.semester_id, old.branch_id);
...
drop trigger if exists trg_reconcile_future_semester_dates on public.semesters;
create constraint trigger trg_reconcile_future_semester_dates
after update on public.semesters
deferrable initially deferred
for each row
execute function private.reconcile_semester_row_change();

drop trigger if exists trg_reconcile_future_branch_semester_override on public.branch_semester_overrides;
create constraint trigger trg_reconcile_future_branch_semester_override
after insert or update or delete on public.branch_semester_overrides
deferrable initially deferred
for each row
execute function private.reconcile_branch_semester_override_change();

drop trigger if exists trg_reconcile_future_instructional_break on public.closure_periods;
create constraint trigger trg_reconcile_future_instructional_break
after insert or update or delete on public.closure_periods
deferrable initially deferred
for each row
execute function private.reconcile_instructional_break_change();
